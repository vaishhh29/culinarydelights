...
const SopapillasRecipe = () => {
  ...
  const recipeData = {
    ...
    instructions: [
      ...
      'Add melted butter and warm water. Mix until a dough forms.',
      'Knead the dough on a floured surface until smooth.',
      'Cover and let it rest for 30 minutes.',
      'Roll out the dough to 1/4-inch thickness and cut into squares or triangles.',
      'In a deep pan, heat vegetable oil to 375°F (190°C).',
      'Fry the sopapillas until golden brown on both sides.',
      'Remove and drain on paper towels.',
      'Dust with powdered sugar before serving.',
      'Serve warm with honey. Enjoy your delicious Sopapillas!',
    ],
  };

  const handleLinkPress = () => {
    const url = 'https://youtu.be/GL3VRVFulQ8?si=CsYFFlI0FHE7rMpO';
  ...
  const unsaveRecipe = async () => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', userEmail), where('recipeId', '==', 'sopapillas'));
  ...
  const checkIfRecipeIsSaved = async (email) => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', email), where('recipeId', '==', 'sopapillas'));
      const querySnapshot = await getDocs(q);
    ...
    >
      <ScrollView>
        <View style={styles.container}>
          <Text style={styles.title}>Sopapillas</Text>
          ...
          <View style={styles.con}>
            <Text style={styles.youtube}>
              ...
              <TouchableOpacity onPress={handleLinkPress}>
                <Text style={styles.linkText}>https://youtu.be/GL3VRVFulQ8?si=CsYFFlI0FHE7rMpO</Text>
              </TouchableOpacity>
            </Text>
          </View>
          ...
          <Text style={styles.lasttext}>Enjoy the delicious Sopapillas!</Text>
        </View>
      </ScrollView>
    </ImageBackground>
  );
...
const styles = StyleSheet.create({
    ...
    export default SopapillasRecipe;