...
const MochiRecipe = () => {
  ...
  const recipeData = {
    ...
    ingredients: [
      ...
      'Your favorite sweet fillings (e.g., red bean paste, ice cream)',
    ],
    instructions: [
      'In a bowl, whisk together mochiko, sugar, and water until smooth.',
      'Cover the bowl with plastic wrap and steam the mixture for 30-45 minutes.',
      'Let the mochi mixture cool to room temperature.',
      'Dust a surface with potato starch or cornstarch.',
      'Transfer the mochi mixture to the dusted surface and flatten it with your hands.',
      'Place your favorite sweet fillings in the center of the mochi.',
      'Fold the mochi over the filling and pinch the edges to seal.',
      'Roll the mochi into a ball or cylinder shape.',
      'Repeat the process for the remaining mochi and fillings.',
      'Dust the finished mochi with more potato starch or cornstarch to prevent sticking.',
  ...
  const unsaveRecipe = async () => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', userEmail), where('recipeId', '==', 'mochi_recipe'));
  ...
  const checkIfRecipeIsSaved = async (email) => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', email), where('recipeId', '==', 'mochi_recipe'));
      const querySnapshot = await getDocs(q);
    ...
    >
      <ScrollView>
        <View style={styles.container}>
          ...
          <Text style={styles.lasttext}>Enjoy making and savoring your homemade mochi!</Text>
        </View>
      </ScrollView>
    </ImageBackground>
  );
