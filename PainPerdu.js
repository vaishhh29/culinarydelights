...
const PainPerduRecipe = () => {
  ...
  const recipeData = {
    ...
    instructions: [
      ...
      'Place the coated bread slices on the hot skillet and cook until golden brown on both sides.',
      'Serve the Pain Perdu with maple syrup and fresh berries.',
      'Enjoy your delicious Pain Perdu!',
    ],
  };

  const handleLinkPress = () => {
    const url = 'https://youtu.be/SlimJHkGt-o?si=wWNy-idXXIRsKUoM';
  ...
  const unsaveRecipe = async () => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', userEmail), where('recipeId', '==', 'painperdu'));
  ...
  const checkIfRecipeIsSaved = async (email) => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', email), where('recipeId', '==', 'painperdu'));
      const querySnapshot = await getDocs(q);
    ...
    >
      <ScrollView>
        <View style={styles.container}>
          <Text style={styles.title}>Pain Perdu</Text>
          ...
          <View style={styles.con}>
            <Text style={styles.youtube}>
              ...
              <TouchableOpacity onPress={handleLinkPress}>
                <Text style={styles.linkText}>https://youtu.be/SlimJHkGt-o?si=wWNy-idXXIRsKUoM</Text>
              </TouchableOpacity>
            </Text>
          </View>
          ...
          <Text style={styles.lasttext}>Enjoy the delicious Pain Perdu!</Text>
        </View>
      </ScrollView>
    </ImageBackground>
  );
...
export default PainPerduRecipe;
