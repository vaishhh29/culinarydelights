...
const ChilaquilesRecipe = () => {
  ...
  const recipeData = {
    ...
    instructions: [
      ...
      'In a bowl, mix the shredded chicken with salsa. Spoon the chicken mixture over the tortilla chips.',
      'Drop dollops of sour cream over the chicken mixture.',
      'Sprinkle shredded cheese evenly over the top.',
      'Bake in the preheated oven for about 15 minutes or until the cheese is melted and bubbly.',
      'Remove from the oven and let it cool slightly.',
      'Garnish with chopped cilantro, sliced jalapeños, diced tomatoes, sliced green onions, and avocado slices.',
      'Season with salt and pepper to taste.',
      'Serve immediately and enjoy your delicious Chilaquiles!',
    ],
  };

  const handleLinkPress = () => {
    const url = 'https://youtu.be/JlmfX8Zv9FA?si=REzGMpa-StFjozeZ';
  ...
  const unsaveRecipe = async () => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', userEmail), where('recipeId', '==', 'chilaquiles_recipe'));
  ...
  const checkIfRecipeIsSaved = async (email) => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', email), where('recipeId', '==', 'chilaquiles_recipe'));
      const querySnapshot = await getDocs(q);
    ...
    >
      <ScrollView>
        <View style={styles.container}>
          <Text style={styles.title}>Chilaquiles</Text>
          ...
          <View style={styles.con}>
            <Text style={styles.youtube}>
              ...
              <TouchableOpacity onPress={handleLinkPress}>
                <Text style={styles.linkText}>https://youtu.be/JlmfX8Zv9FA?si=REzGMpa-StFjozeZ</Text>
              </TouchableOpacity>
            </Text>
          </View>
          ...
          <Text style={styles.lasttext}>Enjoy your delicious Chilaquiles!</Text>
        </View>
      </ScrollView>
    </ImageBackground>
  );
...
export default ChilaquilesRecipe;
