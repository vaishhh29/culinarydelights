...
const PainAuChocolatRecipe = () => {
  ...
  const recipeData = {
    ...
    instructions: [
      ...
      'Roll out the puff pastry sheet on a lightly floured surface to smooth out creases.',
      'Cut the pastry into rectangles, about 3x4 inches in size.',
      'Place a small amount of chopped chocolate at one end of each rectangle.',
      'Roll the pastry over the chocolate to create a log, making sure the seam is on the bottom.',
      'Place the rolled pastries on a baking sheet lined with parchment paper.',
      'Brush the tops of the pastries with the beaten egg for a shiny finish.',
      'Bake in the preheated oven for 15-20 minutes or until golden brown and puffed up.',
      'Remove from the oven and let them cool slightly.',
      'Optional: Dust with powdered sugar before serving.',
      'Enjoy your delicious Pain au Chocolat!',
    ],
  };

  const handleLinkPress = () => {
    const url = 'https://youtu.be/UtpBjLiMlWI?si=eQg3pzo-Y-3jDtI1';
  ...
  const unsaveRecipe = async () => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', userEmail), where('recipeId', '==', 'painauchocolat'));
  ...
  const checkIfRecipeIsSaved = async (email) => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', email), where('recipeId', '==', 'painauchocolat'));
      const querySnapshot = await getDocs(q);
    ...
    >
      <ScrollView>
        <View style={styles.container}>
          <Text style={styles.title}>Pain au Chocolat</Text>
          ...
          <View style={styles.con}>
            <Text style={styles.youtube}>
              ...
              <TouchableOpacity onPress={handleLinkPress}>
                <Text style={styles.linkText}>https://youtu.be/UtpBjLiMlWI?si=eQg3pzo-Y-3jDtI1</Text>
              </TouchableOpacity>
            </Text>
          </View>
          ...
          <Text style={styles.lasttext}>Enjoy the delicious Pain au Chocolat!</Text>
        </View>
      </ScrollView>
    </ImageBackground>
  );
...
const styles = StyleSheet.create({
    ...
    export default PainAuChocolatRecipe;