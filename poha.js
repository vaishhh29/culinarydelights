...
const PohaRecipe = () => {
  ...
  const recipeData = {
    ...
    instructions: [
      'Wash the flattened rice (poha) in cold water and set it aside to drain excess water.',
      'Heat oil in a pan, add mustard seeds, cumin seeds, asafoetida, and let them splutter.',
      'Add chopped onions, green chilies, curry leaves, and sauté until onions turn translucent.',
      'Add diced potatoes and green peas. Cook until the potatoes are tender.',
      'Add turmeric powder and salt. Mix well.',
      'Add the washed flattened rice (poha) to the pan. Mix gently to combine with the other ingredients.',
      ...
      'Serve hot Poha with your favorite chutney or yogurt.',
    ],
  };

  const handleLinkPress = () => {
    const url = 'https://youtu.be/xvGOwYnACU8?si=njnNSPmaeeLMPc57';
  ...
  const unsaveRecipe = async () => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', userEmail), where('recipeId', '==', 'poha'));
  ...
  const checkIfRecipeIsSaved = async (email) => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', email), where('recipeId', '==', 'poha'));
      const querySnapshot = await getDocs(q);
    ...
    >
      <ScrollView>
        <View style={styles.container}>
          ...
          <View style={styles.con}>
            <Text style={styles.youtube}>
              ...
              <TouchableOpacity onPress={handleLinkPress}>
                <Text style={styles.linkText}>https://youtu.be/xvGOwYnACU8?si=njnNSPmaeeLMPc57</Text>
              </TouchableOpacity>
            </Text>
          </View>
          ...
          <Text style={styles.lasttext}>Enjoy your delicious Poha!</Text>
        </View>
      </ScrollView>
    </ImageBackground>
  );
