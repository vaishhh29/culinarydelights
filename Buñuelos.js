...
const BunuelosRecipe = () => {
  ...
  const recipeData = {
    ...
    instructions: [
      ...
      'In a separate bowl, mix together milk, melted butter, egg, and vanilla extract.',
      'Add the wet ingredients to the dry ingredients and stir until a dough forms.',
      'Knead the dough on a floured surface until smooth.',
      'Cover and let it rest for 30 minutes.',
      'Roll out the dough to 1/8-inch thickness and cut into rounds or desired shapes.',
      'In a deep pan, heat vegetable oil to 350°F (175°C).',
      'Fry the buñuelos until golden brown on both sides.',
      'Remove and drain on paper towels.',
      'Coat with cinnamon sugar while still warm.',
      'Serve and enjoy your delicious Buñuelos!',
    ],
  };

  const handleLinkPress = () => {
    const url = 'https://youtu.be/JhSmG8DzNBo?si=FcReup_AUb2NPomC';
  ...
  const unsaveRecipe = async () => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', userEmail), where('recipeId', '==', 'bunuelos'));
  ...
  const checkIfRecipeIsSaved = async (email) => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', email), where('recipeId', '==', 'bunuelos'));
      const querySnapshot = await getDocs(q);
    ...
    >
      <ScrollView>
        <View style={styles.container}>
          <Text style={styles.title}>Buñuelos</Text>
          ...
          <View style={styles.con}>
            <Text style={styles.youtube}>
              ...
              <TouchableOpacity onPress={handleLinkPress}>
                <Text style={styles.linkText}>https://youtu.be/JhSmG8DzNBo?si=FcReup_AUb2NPomC</Text>
              </TouchableOpacity>
            </Text>
          </View>
          ...
          <Text style={styles.lasttext}>Enjoy the delicious Buñuelos!</Text>
        </View>
      </ScrollView>
    </ImageBackground>
  );
