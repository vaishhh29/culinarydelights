...
const KheerRecipe = () => {
  ...
  const recipeData = {
    ...
    instructions: [
      ...
      'Drain the soaked rice and add it to the boiling milk. Simmer and cook until the rice is fully cooked and the milk has thickened.',
      'Add sugar, chopped nuts, raisins, cardamom powder, and saffron strands. Mix well and continue to cook until the sugar is fully dissolved and the kheer reaches a creamy consistency.',
      'Remove from heat and let it cool. The kheer will thicken further as it cools.',
      'Serve chilled, garnished with additional nuts and saffron strands if desired.',
    ],
  };

  const handleLinkPress = () => {
    const url = 'https://youtu.be/4rzA0-WYbFA?si=Asd3D8yvv3ED_ylM';
  ...
  const unsaveRecipe = async () => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', userEmail), where('recipeId', '==', 'kheer'));
  ...
  const checkIfRecipeIsSaved = async (email) => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', email), where('recipeId', '==', 'kheer'));
      const querySnapshot = await getDocs(q);
    ...
    >
      <ScrollView>
        <View style={styles.container}>
          <Text style={styles.title}>Kheer</Text>
          ...
          <View style={styles.con}>
            <Text style={styles.youtube}>
              ...
              <TouchableOpacity onPress={handleLinkPress}>
                <Text style={styles.linkText}>https://youtu.be/4rzA0-WYbFA?si=Asd3D8yvv3ED_ylM</Text>
              </TouchableOpacity>
            </Text>
          </View>
          ...
          <Text style={styles.lasttext}>Enjoy the delicious Kheer!</Text>
        </View>
      </ScrollView>
    </ImageBackground>
  );
