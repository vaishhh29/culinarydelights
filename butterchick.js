...
const ButterChickenRecipe = () => {
  ...
  const recipeData = {
    ...
    instructions: [
      'Marinate chicken pieces in yogurt, ginger-garlic paste, red chili powder, turmeric powder, garam masala, and salt. Let it marinate for at least 2 hours or overnight.',
      'In a pan, melt butter and add marinated chicken. Cook until the chicken is browned.',
      'Add tomato puree and cook until the oil separates.',
      'Add cream and kasuri methi. Simmer for 10-15 minutes until the chicken is fully cooked.',
      'Garnish with coriander leaves and serve hot with naan or rice.',
    ],
  };

  const handleLinkPress = () => {
    const url = 'https://youtu.be/EO30A86XSdM?si=C9CPNVA4LFEMuz7U';
  ...
  const unsaveRecipe = async () => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', userEmail), where('recipeId', '==', 'butter_chicken'));
  ...
  const checkIfRecipeIsSaved = async (email) => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', email), where('recipeId', '==', 'butter_chicken'));
      const querySnapshot = await getDocs(q);
    ...
    >
      <ScrollView>
        <View style={styles.container}>
          ...
          <View style={styles.con}>
            <Text style={styles.youtube}>
              ...
              <TouchableOpacity onPress={handleLinkPress}>
                <Text style={styles.linkText}>https://youtu.be/EO30A86XSdM?si=C9CPNVA4LFEMuz7U</Text>
              </TouchableOpacity>
            </Text>
          </View>
          ...
          <Text style={styles.lasttext}>Enjoy your delicious Butter Chicken!</Text>
        </View>
      </ScrollView>
    </ImageBackground>
  );
