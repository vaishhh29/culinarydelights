...
const TamaRecipe = () => {
  ...
  const recipeData = {
    ...
    instructions: [
      ...
      'Crack a raw egg over the rice. Be sure that the rice is hot enough to slightly cook the egg.',
      'Add soy sauce and salt to taste.',
      'Mix everything together until the egg is slightly cooked and the rice is well coated.',
  ...
  const handleLinkPress = () => {
    const url = 'https://youtu.be/YVS6nodKlIc?si=HAUHFSUN-aReN1OF';
  ...
  const unsaveRecipe = async () => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', userEmail), where('recipeId', '==', 'tamago_kake_gohan'));
  ...
  const checkIfRecipeIsSaved = async (email) => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', email), where('recipeId', '==', 'tamago_kake_gohan'));
      const querySnapshot = await getDocs(q);
    ...
    >
      <ScrollView>
        <View style={styles.container}>
          <Text style={styles.title}>Tamago Kake Gohan</Text>

          <Text style={styles.sectionText}>Duration: 5 minutes</Text>

          <View style={styles.con}>
            <Text style={styles.youtube}>
              YOUTUBE LINK:
              <TouchableOpacity onPress={handleLinkPress}>
                <Text style={styles.linkText}>https://youtu.be/YVS6nodKlIc?si=HAUHFSUN-aReN1OF</Text>
              </TouchableOpacity>
            </Text>
          </View>
          ...
          <Text style={styles.lasttext}>Enjoy your Tamago Kake Gohan!</Text>
        </View>
      </ScrollView>
    </ImageBackground>
  );
