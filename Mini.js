...
const MiniQuichesRecipe = () => {
  ...
  const recipeData = {
    ...
    instructions: [
      ...
      'Using a round cookie cutter or glass, cut out circles from the puff pastry and press them into a greased mini muffin tin.',
      'In a bowl, whisk together eggs, milk, salt, and pepper.',
      'Divide shredded cheese, diced ham or bacon, chopped onion, and bell pepper evenly among the pastry cups.',
      ...
      'Garnish with fresh chives and serve these delicious Mini Quiches!',
    ],
  };

  const handleLinkPress = () => {
    const url = 'https://youtu.be/jDd_KTi4HEg?si=m9MtermXSyCQthyr';
  ...
  const unsaveRecipe = async () => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', userEmail), where('recipeId', '==', 'miniquiches'));
  ...
  const checkIfRecipeIsSaved = async (email) => {
    try {
      const savedCollection = collection(firestore, 'saved');
      const q = query(savedCollection, where('userEmail', '==', email), where('recipeId', '==', 'miniquiches'));
      const querySnapshot = await getDocs(q);
    ...
    >
      <ScrollView>
        <View style={styles.container}>
          <Text style={styles.title}>Mini Quiches</Text>
          ...
          <View style={styles.con}>
            <Text style={styles.youtube}>
              ...
              <TouchableOpacity onPress={handleLinkPress}>
                <Text style={styles.linkText}>https://youtu.be/jDd_KTi4HEg?si=m9MtermXSyCQthyr</Text>
              </TouchableOpacity>
            </Text>
          </View>
          ...
          <Text style={styles.lasttext}>Enjoy these delightful Mini Quiches!</Text>
        </View>
      </ScrollView>
    </ImageBackground>
  );
...
export default MiniQuichesRecipe;
